...
#+SUBTITLE: for version 2.90.1 (v2.90.1-554-gaf28fc1e6+1)
...
#+TEXINFO: @noindent
This manual is for Magit version 2.90.1 (v2.90.1-554-gaf28fc1e6+1).
...
log buffer, depending on the value of ~magit-use-sticky-arguments~ (see
[[*Transient Commands]]).
...
buffer, depending on the value of ~magit-use-sticky-arguments~ (see
[[*Transient Commands]]).
...
- User Option: magit-clone-set-remote.pushDefault
  ...
  - If ~ask~, then the users are asked every time they clones a
    repository.
