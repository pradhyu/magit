               ...
               (let ((branch (cdr (magit-split-branch-name ref))))
                 (if (magit-branch-p branch)
                     (if (magit-rev-eq branch ref)
                       ...
                       (setq branch (propertize branch 'font-lock-face 'magit-branch-local))
                       (setq ref (propertize ref 'font-lock-face 'magit-branch-remote))
    ...
    (magit-insert-section (remote remote)
      (magit-insert-heading
        (let ((pull (magit-get "remote" remote "url"))
          ...
          (format (propertize "Remote %s (%s):" 'font-lock-face 'magit-section-heading)
      ...
      (let* ((headp (equal head "*"))
             ...
             (branch-desc
              (if branch
                ...
                (propertize "(detached)" 'font-lock-face 'font-lock-warning-face)))
             (u:ahead  (and u:track
        ...
        (list (1+ (length (concat branch-desc u:ahead p:ahead u:behind)))
              ...
              (and pushp
                   (concat p:behind
                           (propertize push 'font-lock-face 'magit-branch-remote)
                           " "))
              (and msg (magit-log-propertize-keywords nil msg)))))))
...
(defun magit-refs--propertize-branch (branch ref &optional head-face)
  (let ((face (cdr (cl-find-if (pcase-lambda (`(,re . ,_))
    ...
    (propertize branch 'font-lock-face (if head-face (list face head-face) face))))
