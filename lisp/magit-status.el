...
the status buffer causes this section to disappear again."
  (when magit-this-error
    (magit-insert-section (error 'git)
      ...
      (insert (propertize magit-this-error 'font-lock-face 'font-lock-warning-face))
         ...
         (if-let ((upstream (or upstream (magit-get-upstream-branch branch))))
           ...
           (cond
            ((magit--unnamed-upstream-p remote merge)
             (concat (propertize merge  'font-lock-face 'magit-branch-remote) " from "
                     (propertize remote 'font-lock-face 'bold)))
            ((magit--valid-upstream-p remote merge)
             (if (equal remote ".")
                 (concat
                  (propertize merge 'font-lock-face 'magit-branch-local)
                  (propertize " does not exist" 'font-lock-face 'font-lock-warning-face))
               (concat
                (propertize merge 'font-lock-face 'magit-branch-remote)
                (propertize " does not exist on " 'font-lock-face 'font-lock-warning-face)
